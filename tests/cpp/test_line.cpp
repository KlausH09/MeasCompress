...
TEST_CASE("GetY", "[measCompress, line]")
{
    ...
    REQUIRE(line.GetY(T(3)) == Catch::Approx(T(47)));
    REQUIRE(line.GetY(T(0)) == Catch::Approx(T(44)));
    REQUIRE(line.GetY(T(4)) == Catch::Approx(T(48)));
}
...
{
    {
        ...
        REQUIRE(line.GetMaxError(t, y) == Catch::Approx(T(1)));
    }
    {
        T m = 1;
        T y0 = 3;
...
{
    {
        ...
        REQUIRE(line.GetY(T(0)) == Catch::Approx(T(0)));
        REQUIRE(line.GetY(T(7)) == Catch::Approx(T(0)));
    }
    {
        std::vector<T> t = {1, 1, 2, 2};
        std::vector<T> y = {2, 4, 3, 5};
        auto line = Line<T>::Fit(t, y);
        REQUIRE(line.GetY(T(0)) == Catch::Approx(T(2)));
        REQUIRE(line.GetY(T(1)) == Catch::Approx(T(3)));
        REQUIRE(line.GetY(T(2)) == Catch::Approx(T(4)));
    }
    {
        std::vector<T> t = {1, 1, 2, 2};
